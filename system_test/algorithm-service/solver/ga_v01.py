"""
遗传算法求解器 V01 - 多样化初始化版本
核心改进：
1. 多样化初始化（20%随机 + 30%最近邻 + 20%需求优先 + 15%最远插入 + 15%节约算法）
2. 遗憾值法分配仓库
3. 动态参数调整
4. 灾变机制
5. 借鉴PSO V04的成功经验
...
def farthest_insertion_ga(n_customers: int) -> np.ndarray:
    """
    最远插入启发式（借鉴PSO V04）
    每次选择距离当前路径最远的客户插入
    """
    ...
    distances_from_depot = instance.distance_matrix[depot_idx, c_start:c_start+n_customers]
    farthest_customer = np.argmax(distances_from_depot)
    
    route.append(farthest_customer)
    remaining.remove(farthest_customer)
    
    # 预计算客户间距离
    c_end = c_start + n_customers
    ...
    while remaining:
        ...
        for customer in remaining:
            # 计算到路径的最小距离
            min_dist_to_route = min(cust_dist_matrix[customer, node] for node in route)
    ...
    # 转换为实际索引
    return instance.num_depots + np.array(route)


def savings_algorithm_ga(n_customers: int) -> np.ndarray:
    """
    节约算法启发式（借鉴PSO V04）
    计算合并两条路径的节约值，优先合并节约最大的
    """
    global instance
    ...
    for i in range(n_customers):
        for j in range(i + 1, n_customers):
            ...
            save = (instance.distance_matrix[depot_idx, ci] + 
                   instance.distance_matrix[depot_idx, cj] - 
    ...
    # 转换为实际索引
    return instance.num_depots + np.array(result)


def initialize_population_diverse(pop_size: int) -> np.ndarray:
    """
    多样化初始化策略（借鉴PSO V04的成功经验）
    20%随机 + 30%最近邻 + 20%需求优先 + 15%最远插入 + 15%节约算法
    """
    global instance
    ...
    # 计算每种策略的数量
    n_random = int(pop_size * 0.2)
    n_heuristic = int(pop_size * 0.3)
    n_demand = int(pop_size * 0.2)
    n_farthest = int(pop_size * 0.15)
    n_savings = pop_size - n_random - n_heuristic - n_demand - n_farthest
    ...
    # 4. 最远插入初始化 15%
    for _ in range(n_farthest):
        individual = farthest_insertion_ga(n_customers)
        # 添加随机性
        n_swaps = max(1, int(0.1 * len(individual)))
        for _ in range(n_swaps):
            i1, i2 = np.random.randint(0, len(individual), 2)
            individual[i1], individual[i2] = individual[i2], individual[i1]
        pop_list.append(individual)
    ...
    for _ in range(n_savings):
        individual = savings_algorithm_ga(n_customers)
        # 添加随机性
        n_swaps = max(1, int(0.1 * len(individual)))
        for _ in range(n_swaps):
            i1, i2 = np.random.randint(0, len(individual), 2)
...
class GeneticAlgorithmOptimized:
    ...
    def solve(self) -> Dict[str, Any]:
        ...
        print(f"  多进程: 是")
        print(f"  初始化策略: 20%随机 + 30%最近邻 + 20%需求 + 15%最远插入 + 15%节约算法")
        print(f"{'='*60}\n")

        # 初始化种群（多样化初始化）
        population = initialize_population_diverse(self.population_size)
...
def initialize_population():
    """兼容性函数"""
    global instance
    return initialize_population_diverse(300)



# ==================== API Solver 包装类 ====================

class GeneticAlgorithmV01Solver:
    """多进程遗传算法求解器 - API包装类"""
